/**
 * This is cut down from ideal.c, providing only that which is needed for a
 * header. The Python package will use this.
 */
 ...
 */
#define ideal_pressure(...) ideal_pressure_base( \
    (ideal_struct){.pressure=1, .moles=1, .temp=273.15, __VA_ARGS__})
