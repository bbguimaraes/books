...
/**
 * Frees the original string, because strtok_r mangled it, so it isn't useful
...
int main() {
    char * delimiters = " `~!@#$%^*()_-+={[]}|\\;:\",<>./?\n";
    ok_array * o = ok_array_new(
        strdup("Hello,  reader. This is text."), delimiters);
    assert(o->length == 5);
